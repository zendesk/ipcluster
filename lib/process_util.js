...
function getCommand(pid) {
	return Q.nfcall(fs.readFile, '/proc/' + pid + '/cmdline')
		.then(function(res) {
...
function getEnv(pid) {
	return Q.nfcall(fs.readFile, '/proc/' + pid + '/environ')
		.then(function(res) {
			var env = {};
...
function mem(pid) {
	...
	var smaps_file = mem_smap_location.replace('{pid}', pid);
